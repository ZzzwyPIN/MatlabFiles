...
%     snr
    ...
    symbol_error_nbr = 0;   
    for loop = 1:Num
        init_bit = randi([0 1],Mt*modulation_order,1);%生成4*1的矩阵，每个元素为随机的0或1.
        mod_sym = bi2de(reshape(init_bit,modulation_order,Mt)','left-msb')+1;%加1是因为数组下标从1开始而不是0.
        S = symbols(mod_sym);  % 产生符号序列

        H = normrnd(0,1,Mr,Mt) + 1i*normrnd(0,1,Mr,Mt); % 产生信道矩阵
		N_noise = normrnd(0,No,Mr,1) + 1i*normrnd(0,No,Mr,1); % 产生噪声
